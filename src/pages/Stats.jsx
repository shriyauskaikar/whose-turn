...
function Bar({ value, max, color, label, showLabel = true }) {
  ...
  return (
    <div className="flex items-center gap-2">
      {showLabel && (
        <span className="text-xs font-medium w-16 truncate text-right shrink-0" style={{ color: 'var(--text-secondary)' }}>
          {label}
        </span>
      )}
      <div className="flex-1 h-6 rounded-full overflow-hidden" style={{ backgroundColor: 'var(--bar-bg)' }}>
        <div
      ...
      <span className="text-xs font-bold w-6 text-right" style={{ color: 'var(--text-secondary)' }}>{value}</span>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen pb-24" style={{ backgroundColor: 'var(--bg)' }}>
      ...
      <div className="px-4 pt-4">
        <div className="rounded-2xl p-5 border border-amber-200/40" style={{ backgroundColor: 'var(--card)' }}>
          ...
          <div className="space-y-2">
            ...
            )) || <p className="text-sm" style={{ color: 'var(--text-light)' }}>No entries yet</p>}
          </div>
        </div>
      </div>
      ...
      <div className="px-4 pt-4 space-y-4">
        <h2 className="text-sm font-semibold px-1" style={{ color: 'var(--heading)' }}>Per section</h2>
        {stats?.per_section?.length === 0 && (
          <p className="text-sm px-1" style={{ color: 'var(--text-light)' }}>No sections yet</p>
        )}
